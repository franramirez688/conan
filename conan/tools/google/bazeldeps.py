...
class _BazelPathsGenerator:
    """
    Bazel >= 7.1 needs to know all the dependencies, provided via the MODULE.bazel file.
    Therefore we provide a static repository rule to load the dependencies. This rule is used by a
...
class BazelDeps:
    ...
    def generate(self):
        """
        Generates all the targets <DEP>/BUILD.bazel files, a conan_deps_repo_rules.bzl and a
        conan_deps_module_extension.bzl file in the build folder.

        The ``conan_deps_module_extension.bzl`` file should be loaded by your Module.bazel file,
        e.g. like this:
